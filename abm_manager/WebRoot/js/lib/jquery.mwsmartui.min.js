...
(function($) {
	function buildDialog(target) {
		...
		if (opts.buttons) {
			if ($.isArray(opts.buttons)) {
				...
				var btcontainer = $("<div class=\"dialog-button\" style=\"text-align:"+opts.buttonAlign+";\"></div>").appendTo(win);
				for ( var i = 0; i < opts.buttons.length; i++) {
	...
	$.fn.dialog.parseOptions = function(target) {
		return $.extend( {}, $.fn.window.parseOptions(target), $.parser
				.parseOptions(target, [ "toolbar", "buttons","buttonAlign" ]));
	};
		...
		rules:{
			...
			remote:{
				validator:function(value,remoteKey){
					...
					var ret=$.ajax({url:remoteKey[0],dataType:"json",data:queryParam,async:false,cache:false,type:"post"}).responseText;
					return ret=="true";
				},
		...
		render : function(target, ul, data) {
			...
			function renderNode(indnts, nodes) {
				...
				for ( var i = 0; i < nodes.length; i++) {
					...
					if (opts.checkbox) {
						if ((!opts.onlyLeafCheck) || leafOpen) {
							cc
